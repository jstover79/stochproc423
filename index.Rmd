...
<div class="defbox">
**Definition.** A *stochastic process* $X$ is a collection of random variables indexed by a set $X=(X_t)_{t\in T}$. For each $t\in T$, $X_t$ is a $S$-valued random variable, that is each $X_t$ takes values in $S$. The set $S$ is called the *state space*.
</div>

<div class="defbox">
**Definition.** For stochastic process $X=(X_t)_{t\in T}$ with state space $S$ and index set $T$, the *sample path space* is $\Omega=S^T$, that is, if we know the precise value of $X_t$ for all $t\in T$, then $X$ is a function from $T$ to $S$.
</div>
