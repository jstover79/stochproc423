...
Now to apply the continuity correction is a bit trickier though, because $X_n$ only takes on values $-n,-n+2,\ldots,n-2,n$. Instead of adding and subtracting $\frac12$, we must add and subtract $1$ and $P(X_n=j)$ is approximated by
```{r eval=FALSE, echo=TRUE}
pnorm(j+1,0,sqrt(n))-pnorm(j-1,0,sqrt(n))
```
<!-- ::: -->
